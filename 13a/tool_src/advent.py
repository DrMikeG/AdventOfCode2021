...
def foldInX(gridXMax,gridYMax,coords,foldLine):
    ...
    # Horizontal fold
    # Distance from bottom becomes distance from top
    newCoords =[]
    for coord in coords:
        if coord[1] > foldLine:
            distanceFromBottom = (gridYMax - coord[1])-1
            newCoord = [coord[0],distanceFromBottom]
    ...
    return gridXMax,int(gridYMax/2),newCoords
...
def foldInY(gridXMax,gridYMax,coords,foldLine):
    ...
    for coord in coords:
        if coord[0] > foldLine:
            distanceFromRight = (gridXMax - coord[0])-1
            newCoord = [distanceFromRight,coord[1]]
            if not newCoord in newCoords:
                newCoords.append(newCoord)
        else:
            if not coord in newCoords:
    ...
    return int(gridXMax/2),gridYMax,newCoords
...
def mainTask():
    ...
    input_path = "C:\\Users\\gibbens\\Documents\\Arduino\\AdventOfCode2021\\13a\\tool_src\\input.txt"
    ...
    #gridXMax,gridYMax,coords = foldInX(gridXMax,gridYMax,coords,655)
