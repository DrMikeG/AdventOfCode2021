...
def mainTask():
    ...
    input_path = "C:\\Users\\gibbens\\Documents\\Arduino\\AdventOfCode2021\\15a\\tool_src\\input.txt"
    graph = processInputFile(input_path)
    
    gridXMax=len(graph[0])
    gridYMax=len(graph)
    printGrid(gridXMax,gridYMax,graph)

    # https://levelup.gitconnected.com/dijkstra-algorithm-in-python-8f0e75e3f16e

    parentsMap, nodeCosts = dijkstra(gridXMax,gridYMax, graph, (0,0) )
