...
def processLineOfInputIntoStruct(line,struct):
    # 964,133 -> 596,133
    lineTupleStr = line.strip().replace(" -> ", ",").replace(" ", "")
    lineTuple = lineTupleStr.split(",",4)

    cords = []
    for str in lineTuple:
        cords.append(int(str))
...
def mainTask():
    ...
    input_path = "C:\\Users\\gibbens\\Documents\\Arduino\\AdventOfCode2021\\05b\\tool_src\\input.txt"
    struct = processInputFile(input_path)
    grid = populateGrid(struct)
    for line in struct:
        addLineToGrid(grid,line)
    
    printGrid(grid)
    print(countValuesLargerThan2(grid))
